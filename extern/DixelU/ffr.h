...
inline decltype(errno) fopen_wrap(FILE*& file, const cchar_t* filename, const cchar_t* parameter)
{
...
inline std::pair<__inner_stream_type*, FILE*> open_wide_stream(std_unicode_string filename, const cchar_t* parameter);

template<>
inline std::pair<std::ostream*, FILE*> open_wide_stream<std::ostream, std::ios_base::out>(std_unicode_string filename, const cchar_t* parameter)
{
...
template<>
inline std::pair<std::istream*, FILE*> open_wide_stream<std::istream, std::ios_base::in>(std_unicode_string filename, const cchar_t* parameter)
{
	FILE* file_ptr = nullptr;
...
template<typename __inner_stream_type, decltype(std::ios_base::out) stream_io_type = std::ios_base::out>
inline std::pair<__inner_stream_type*, FILE*> open_wide_stream(std_unicode_string file, const cchar_t* parameter)
{
	FILE* c_file = nullptr;
...
template<decltype(std::ios_base::out) stream_io_type = std::ios_base::out>
inline std::pair<std::ostream*, FILE*> open_wide_stream(
    std_unicode_string file, const cchar_t* parameter)
{
    FILE* c_file = nullptr;
    return {new std::ostream(file.c_str(), parameter), c_file};
}

#else

template<typename __inner_stream_type, decltype(std::ios_base::out) stream_io_type = std::ios_base::out>
inline std::pair<__inner_stream_type*, FILE*> open_wide_stream(std::string file, const cchar_t* parameter);

#endif

typedef struct fast_file_reader
{
private:
	FILE* file_ptr;
...
public:
	fast_file_reader(const cchar_t* filename, int default_buffer_size = 20000000) : true_buffer_size(default_buffer_size)
	{
		auto err_no = fopen_wrap(file_ptr, filename, to_cchar_t("rb"));
	...
	void reopen_next_file(const cchar_t* filename)
	{
		close();
		auto err_no = fopen_wrap(file_ptr, filename, to_cchar_t("rb"));
