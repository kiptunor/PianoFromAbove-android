...
struct single_midi_info_collector 
{
	struct tempo_event
	{
		...
		tempo_event(std::uint8_t a, std::uint8_t b, std::uint8_t c) : a(a), b(b), c(c){}

		tempo_event() : tempo_event(0x7, 0xA1, 0x20){}
	...
	struct note_on_off_counter
	{
		...
		note_on_off_counter(std::int64_t NOn, std::int64_t NOff) : note_on(NOn), note_off(NOff){}
